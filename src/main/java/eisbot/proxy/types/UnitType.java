...
public enum UnitType {
    ...
    public final int cppOrd;

    UnitType(int cppOrd) {
        this.cppOrd = cppOrd;
    }

    private static final java.util.Map<Integer, UnitType> lookup = new HashMap<Integer, UnitType>(UnitType.values().length);

    static {
        for (UnitType ut : UnitType.values()) {
            lookup.put(ut.cppOrd, ut);
        }
    }

    public static UnitType fromId(int id) {
        final UnitType ut = lookup.get(id);
        if (ut == null) {
    ...
    public static int toId(UnitType type) {
        return type.cppOrd;
    }
}
